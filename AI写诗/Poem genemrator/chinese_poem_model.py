...
class PoetryModel(object):
    ...
    def build_model(self):
        ...
        dropout = Dropout(0.4)(lstm)
        dense = Dense(len(self.words), activation='sigmoid')(dropout)
        self.model = Model(inputs=input_tensor, outputs=dense)
        # optimizer = adam_v2(learning_rate=self.config.learning_rate)
...
def predict_next():
    for i in range(3):
        #给出第一句话进行预测
        sentence = input("Input a short sentence to predict: ")
        sen = model.predict_sen(sentence)
        print(sen)

def head_hidden():
    for i in range(3):
        # 藏头诗
        sentence = input("Input 4 Words: ")
        sen = model.predict_hide(sentence)
        print(sen)
...
def one_word():
    sentence = input("Input only 1 word to continue: ")
    for i in range(3):
        # 给出第一个字进行预测
        sen = model.predict_first(sentence)
        print(sen)